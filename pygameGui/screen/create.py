...
def create_menu(leftup=(10, 10), rightdown=(100, 100), color1=gray, color2=litegray, color3=litergray, color4=liteblue, divisions=[1], names=['name'], isVertical=True, nameSize=30, settingValues=None, Font='freesansbold.ttf', fontTitelSize=20, fontSettingSize=15, settings=[{"Hoogte": "10", 'snelheid': '17'}, {'1' : 1, "2": '2'}]):
    ...
    while divisioncount < len(divisions):
        ...
        if isVertical == True:
            space = (rightdown[1] - leftup[1] - (len(divisions) * 5)) * (divisions[divisioncount] / totalDivNumber)
            ...
            space = (rightdown[0] - leftup[0]) * (divisions[divisioncount] / totalDivNumber)
            create_rectangle((leftup[0] + totalSpace, leftup[1] + 5),(leftup[0] + totalSpace + space - 10, rightdown[1] - 5), color2)
            create_rectangle((leftup[0] + totalSpace + 3, leftup[1] + 8),(leftup[0] + totalSpace + space - 13, leftup[1] + nameSize), color3)
...
def create_ellipse(center = (200, 100), radiusHeight= 100, radiusWidth = 50, width=4, color = white):
    leftup = (center[0] - radiusWidth, center[1] - radiusHeight)

    rect = pygame.Rect(leftup[0], leftup[1], radiusWidth * 2, radiusHeight * 2) 
